...
class SchoolTest < Minitest::Test
  ...
  def test_students_by_grade
    ...
    students = %w(Beemee Aimee Ceemee)
    students.each { |student| school.add(student, grade) }
    expected = [{grade: grade, students_by_grades: students.sort }]
    assert_equal expected, school.students_by_grade
  end
  ...
  def test_students_by_grade_sorted
    ...
    everyone.each do |grade|
      grade[:students_by_grades].each { |student| school.add(student, grade[:grade]) }
    end
  ...
  def everyone
    [
      {grade: 3, students_by_grades: %w(Deemee Eeemee) },
      {grade: 1, students_by_grades: %w(Effmee Geemee) },
      {grade: 2, students_by_grades: %w(Aimee Beemee Ceemee) }
    ]
  end
